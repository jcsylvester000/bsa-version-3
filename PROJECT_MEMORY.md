...
_Last updated: 2026-09-23 — after Fix Batch 4._
...
- Tests: 334/334 (vitest). Typecheck clean except a pre-existing cast in `tests/unit/analysisContext.test.ts:117`.
...
| 5 | Hygiene: dead code (IntakeWizard, TriangulationOverlay, rankWhiteSpace v1, pgvector unused), root repro/verify scripts, local-FS storage on Netlify, two report systems (/reports vs Analysis PDF), test type error (`analysisContext.test.ts:117`), White-Space full-table scan per site, README/docs refresh, handoff checklist | ⏳ next |
